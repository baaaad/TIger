...
class BertLMPredictionHead_Tagger(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        super(BertLMPredictionHead_Tagger, self).__init__()
        self.transform = BertPredictionHeadTransform(config)
        self.tagger_num = config.tagger_class_num
        # The output weights are the same as the input embeddings, but there is
        # an output-only bias for each token.
        self.decoder = nn.Linear(
        ...
        self.bias = nn.Parameter(torch.zeros(self.tagger_num))
...
class BertPreTrainingHeads_Tagger(nn.Module):
    def __init__(self, config, bert_model_embedding_weights):
        super(BertPreTrainingHeads_Tagger, self).__init__()
        self.predictions = BertLMPredictionHead(config, bert_model_embedding_weights)
        self.tagger_predictions = BertLMPredictionHead_Tagger(config, bert_model_embedding_weights)
        self.bi_seq_relationship = nn.Linear(config.bi_hidden_size, 2)
        self.imagePredictions = BertImagePredictionHead(config)
        self.fusion_method = config.fusion_method
        self.dropout = nn.Dropout(0.1)
    ...
    def forward(
        ...
        prediction_scores_t = self.predictions(sequence_output_t)
        tagger_prediction_scores_t = self.tagger_predictions(sequence_output_t)
        seq_relationship_score = self.bi_seq_relationship(pooled_output)
        prediction_scores_v = self.imagePredictions(sequence_output_v)

        return prediction_scores_t, prediction_scores_v, seq_relationship_score, tagger_prediction_scores_t
...
class VILBertForVLTasks(BertPreTrainedModel):
    def __init__(self, config, num_labels, dropout_prob=0.1, default_gpu=True):
        ...
        self.cls = BertPreTrainingHeads_Tagger(
            config, self.bert.embeddings.word_embeddings.weight
        )
    ...
    def forward(
        ...
        linguisic_prediction, vision_prediction, vil_binary_prediction, tagger_prediction_scores_t = self.cls(
            sequence_output_t, sequence_output_v, pooled_output_t, pooled_output_v
        )

        return linguisic_prediction, tagger_prediction_scores_t
