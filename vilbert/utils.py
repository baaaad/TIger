...
class tbLogger(object):
    ...
    def step_val(self, epochId, loss, score, task_id, batch_size, split):
        self.task_loss_val[task_id] += loss
        self.task_score_val[task_id] += score * batch_size
        self.task_step_val[task_id] += self.gradient_accumulation_steps
        self.task_datasize_val[task_id] += batch_size
...
def build_insertion_tokens(
        ...
        masked_tokens2 ['[CLS]', 'a', '[MASK]', 'man', '[MASK]', '[MASK]', '[MASK]', 'next', 'to', 'a', '[MASK]', '[MASK]', '[MASK]', '[SEP]']
        target_tokens2 ['[CLS]', 'a', 'young', 'man', 'holding', 'an', 'umbrella', 'next', 'to', 'a', 'herd', 'of', 'cattle', '[SEP]']
        Args:
        ...
        # added MASK tokens.
        masked_tokens = masked_tokens[:_max_seq_length]
        target_tokens = target_tokens[:_max_seq_length]
