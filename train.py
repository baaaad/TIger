...
from vilbert.optimization import BertAdam, Adam, Adamax, WarmupLinearSchedule, WarmupConstantSchedule
...
def main():
    ...
    parser.add_argument(
        "--from_pretrained",
        default="pretrained_models/ViLBERT-6-Layer/pytorch_model_8.bin",
        type=str,
        help="Bert pre-trained model selected in the list: bert-base-uncased, "
             "bert-large-uncased, bert-base-cased, bert-base-multilingual, bert-base-chinese.",
    )
    ...
    parser.add_argument(
        "--learning_rate", default=2e-6, type=float, help="The initial learning rate for Adam."
    )
    parser.add_argument(
        "--tagger_loss_ratio",
        default='1.5',
        type=float,
        help="loss weight ratio for taggers.",
    )
    parser.add_argument(
        "--num_train_epochs",
        default=20,
    ...
    parser.add_argument(
        "--lr_scheduler", default='warmup_linear', type=str, help="whether use learning rate scheduler."
    )
    parser.add_argument(
    ...
    tbLogger = utils.tbLogger(timeStamp, savePath, task_names, task_ids, task_num_iters,
    ...
    num_train_optimization_steps = max(
        task_num_iters.values()) * args.num_train_epochs // args.gradient_accumulation_steps
    ...
    for epochId in tqdm(range(args.num_train_epochs), desc="Epoch"):
        ...
        for step in range(max_num_iter):
            ...
            for task_id in task_ids:
                if iterId >= task_start_iter[task_id]:
                    ...
                    loss, score = ForwardModelsTrain(args, task_cfg, device, task_id, task_count, task_iter_train,
                                                     task_dataloader_train, model, task_losses, task_start_iter)
                    ...
                    if (step + 1) % args.gradient_accumulation_steps == 0:
                        ...
                        if default_gpu:
                            tbLogger.step_train(epochId, iterId, float(loss), float(score), optimizer.show_lr(),
                                                task_id, 'train')
        ...
        if default_gpu:
            ...
            if ave_score > best_score:
                ...
                output_model_file_best = os.path.join(savePath, "pytorch_model_best" + ".bin")
                torch.save(model_to_save.state_dict(), output_model_file_best)
